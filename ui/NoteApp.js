// NOTE: This is the full restored NoteApp.js with all UI components included.
// Includes: Add/Edit, List, Viewer, Settings modal, Error modal, Server modal, Theme toggle

import React, { useState, useEffect, useRef, useCallback } from "react";
import { View, Text, StyleSheet, Button, useColorScheme, Modal, TextInput, Pressable, ScrollView, Switch } from "react-native";
...
import {testConnection, useNoteSync} from "../hooks/useNoteSync";
...
const deleteNote = (id) => {
  const updatedNotes = notes.map(n => n.id === id ? { ...n, deleted: true, updatedAt: Date.now(), username: n.username } : n);
...
const startEdit = (note) => {
  ...
  const safeTags = (note.tags || []).map(t => String(t).trim()).filter(Boolean);
...
const visibleNotes = activeTag
    ? notes.filter(n => n.tags?.includes(activeTag) && !n.deleted && (n.username === username || n.username === "All"))
    : notes.filter(n => !n.deleted && (n.username === username || n.username === "All"));
      ...
      <View style={styles.container}>
        <View style={{ position: "absolute", top: 20, right: 20, zIndex: 5 }}>
          <Pressable
              ...
              style={{ borderWidth: 1, borderColor: "#ccc", backgroundColor: "transparent", paddingVertical: 6, paddingHorizontal: 12, borderRadius: 4 }}>
        ...
        {activeTag && (
            <View style={styles.filterBanner}>
              ...
              <Text onPress={() => setActiveTag(null)} style={styles.clearFilter}>✕ Clear Filter</Text>
            </View>
        )}
        ...
        <Modal visible={configMenuVisible} transparent animationType="fade">
          <View style={styles.modalOverlay}>
            <View style={styles.modal}>
              ...
              <View style={{ height: 10 }} />
              <Button title="View Error Log" onPress={() => { setConfigMenuVisible(false); setShowAdminErrors(true); }} />
              <View style={{ marginVertical: 10 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                  ...
                  <Switch
                      ...
                      onValueChange={(manual) => setThemeOverride(manual ? "dark" : null)}
                  />
                </View>
                {themeOverride !== null && (
                    <Button
                        title={themeOverride === "dark" ? "Switch to Light" : "Switch to Dark"}
                        onPress={() =>
